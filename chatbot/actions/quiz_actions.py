...
import sqlite3
from actions.chatbot_utils import*
import os
...
#                try:
                ...
                sql1 = f'SELECT questions.question_id, question_text, answer_text FROM quizs, questions, answers WHERE quizs.quiz_id == questions.quiz_id AND quizs.quiz_id == {quiz_number} AND questions.question_id == answers.question_id AND is_correct == 1'
                cursor.execute(sql1)
                row = cursor.fetchall()
                logger.info(f"[{tracker.sender_id}] {__file__} :  log question id right ------------------------------ | {row[0][0]} ")
                question_id = row[0][0]
                cursor.execute("UPDATE questions SET user_right_answer = user_right_answer + 1 WHERE question_id = ?", (question_id,))
                cursor.execute("UPDATE questions SET user_email_right_answer = user_email_right_answer || ',' || ? WHERE question_id = ?", (email, question_id,))
                sqliteConnection.commit()
                cursor.execute("SELECT user_right_answer FROM questions WHERE question_id = ?", (question_id,))
                right_an = cursor.fetchall()
                logger.info(f"[{tracker.sender_id}] {__file__} :  log right answer ------------------------------ | {right_an} ")
            else:
...
#                try:
                ...
                row = cursor.fetchall()
                question_id=row[0][0]
                logger.info(f"[{tracker.sender_id}] {__file__} :  log question id wrong ------------------------------ | {row[0][0]} ")
                cursor.execute("UPDATE questions SET user_wrong_answer = user_wrong_answer + 1 WHERE question_id = ?", (question_id,))
                cursor.execute("UPDATE questions SET user_email_wrong_answer = user_email_wrong_answer || ',' || ? WHERE question_id = ?", (email, question_id,))
                sqliteConnection.commit()
            else:
                dispatcher.utter_message(text=f"Register yourself to see the results")
...
class AskQuizOver(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if quiz_question_count == 10:
           sqliteConnection = sqlite3.connect('/home/chatbot/chatbot.db')
           cursor = sqliteConnection.cursor()
           sender = tracker.sender_id
           #questions = selected_quwstion_ids
           options = generate_quiz(sender,cursor)
