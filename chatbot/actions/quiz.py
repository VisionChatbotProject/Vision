...
This form is to launch quiz for a user
    - this form has 3 slots
        ...
        - quiz_number
        - quiz_over
    - action_quiz_number: This action class will ask user to which quiz he wants to go for the selected course
    - action_quiz_over: This action class will iterate over all the questions in the quiz
...
class QuizNumber(Action):
    ...
    def name(self) -> Text:
        return "action_quiz_number"
    def run(self, dispatcher: CollectingDispatcher,
            tracker: Tracker,
            domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:

        quiz_course = ""
        listofQuiz = DB.getListofQuiz(tracker)
        if len(listofQuiz) == 0:
