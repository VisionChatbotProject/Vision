...
if __name__ == "__main__":
    try:
        ...
        # db = getDB(db_name)
        # df = pd.read_sql_query("SELECT * from course", client)
        # print(df['name'].values[0])
        # print(len(df))

        # entry = findOne(tbl="course", name='Language')
        # print(entry)
        #
        # entry = findOne(tbl="course", name='Mathematics')
        # print(entry)
        #
        # entry = findOne(tbl="course", name='Hindi')
        # print(entry)
        #
        # entry = getListofCourses()
        # print(entry)
        #
        # entry = findReferences(course = "Language")
        # print(entry)
        #
        original_question_list = getListofQuizQuestions("Language", 12)
        print(original_question_list)
        # entry, entry1 = utils.helper.shuffleQuestions(entry)
        # print(entry1)
        #
        entry = checkQuizAns("Language", 12, 1, "China,Nepal", original_question_list)
        print(entry)
        entry = checkQuizAns("Language", 12, 2, "Nepal", original_question_list)
        print(entry)
        entry = checkQuizAns("Language", 12, 3, "Nepal", original_question_list)
        print(entry)        #
        entry = insertScore(1, 1, "user@college.com")
        print(entry)
        #
        # entry = get_user('irfanc@gmail.com')
        # print(entry)
        #
        entry = getScore("user1@college.com")
        print(entry)

        # entry = getListofQuiz("abc")
        # print(entry)

        entry = getTeacherEmail()
        print(entry)
